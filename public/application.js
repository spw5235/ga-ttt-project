...
/***/ function(module, exports, __webpack_require__) {
	...
	var nextPlayerFunc = function nextPlayerFunc(currPlayer, playerX, playerO) {
	...
	var currentPlayerVal = function currentPlayerVal(currPlayer, playerX) {
	  if (currPlayer === playerX) {
	    return 1;
	  } else {
	    return 2;
	...
	var messageText = function messageText(nextPlayer) {
	  return "Player " + nextPlayer + ", it's your turn";
	};

	var currPlayerTurn = function currPlayerTurn(arr, playerX, playerO) {
	  var tempArr = [];
	  var currPlayer = void 0;
	...
	var symbolValue = function symbolValue(player, divClassNum, useBoard, tempSymbol, playerX) {
	  if (player === playerX) {
	...
	var isCellEmpty = function isCellEmpty(cellNum, gameBoard) {
	...
	var isBoardFilled = function isBoardFilled(arr) {
	  for (var i = 0; i < arr.length; i++) {
	    if (arr[i] === "") {
	      return false;
	    }
	...
	var isWinner = function isWinner(arr) {
	  var tempArrayX = [];
	  var tempArrayY = [];
	  ...
	  for (var _i = 0; _i < combineArr.length; _i++) {
	    var zeroTrue = false;
	    var oneTrue = false;
	    var twoTrue = false;
	    var thisGameOver = false;
	    var valueZero = combineArr[_i][0].toString();
	    var valueOne = combineArr[_i][1].toString();
	    var valueTwo = combineArr[_i][2].toString();

	    for (var j = 0; j < tempArrayX.length; j++) {
	  ...
	  for (var _i2 = 0; _i2 < combineArr.length; _i2++) {
	    var _zeroTrue = false;
	    var _oneTrue = false;
	    var _twoTrue = false;
	    var _thisGameOver = false;
	    var _valueZero = combineArr[_i2][0].toString();
	    var _valueOne = combineArr[_i2][1].toString();
	    var _valueTwo = combineArr[_i2][2].toString();

	    for (var _j = 0; _j < tempArrayY.length; _j++) {
	      if (tempArrayY[_j] === _valueZero) {
	        _zeroTrue = true;
	        //console.log('if conditional - valueZero');
	        //console.log(zeroTrue)
	      } else if (tempArrayY[_j] === _valueOne) {
	        _oneTrue = true;
	        //console.log('if conditional - valone')
	        //console.log(oneTrue)
	      } else if (tempArrayY[_j] === _valueTwo) {
	        //console.log('if conditional - valTwo')
	        _twoTrue = true;
	      }
	    }

	    if (_zeroTrue && _oneTrue && _twoTrue) {
	      _thisGameOver = true;
	      return true;
	    } else {
	      _zeroTrue = false;
	...
	var gameOver = function gameOver(arr) {
	  var isBF = isBoardFilled(arr);
	  var isW = isWinner(arr);
	  if (isBF || isW) {
	    return true;
	  }
	};

	var recordMove = function recordMove(divClassNum, gameBoard) {
	...
	var removeText = function removeText(divString) {
	  var textDiv = $(divString);
	  textDiv.text("");
	};
	...
	var newGameDummyButton = function newGameDummyButton() {
	  $('.player-turn').text('');
	...
	module.exports = {
	  ...
	  nextPlayerFunc: nextPlayerFunc,
	  currentPlayerVal: currentPlayerVal
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var tempSymbol = void 0;
	var nextPlayer = void 0;
	var currPlayNum = void 0;
	...
	var sortStatsFunction = function sortStatsFunction(a, b) {
	  return a - b;
	};

	var calcWinnerStats = function calcWinnerStats(arr) {
	...
	var onGetStats = function onGetStats() {
	  api.getStats().then(function (response) {
	    var win = [];
	    var loss = [];
	    var tie = [];
	    for (var i = 0; i < response.games.length; i++) {
	      ...
	      if (response.games[i].over === true) {
	        ...
	        // console.log('tempObj');
	        // console.log(tempObj);
	        var lastGame = tempObj[tempObj.length - 1];
	        var winnerofGame = calcWinnerStats(lastGame);
	    ...
	    var winRecord = win.length;
	    var lossRecord = loss.length;
	    var tieRecord = tie.length;
	    ...
	    $("#player-two-winner").show();
	    $("#player-tie").show();
	  }).done(ui.onGetStatsSuccess).fail(ui.onGetStatsFail);
	};

	var getPlayerRecord = function getPlayerRecord() {
	  var gameCount = 0;
	  api.getGame().then(function (response) {
	    for (var i = 0; i < response.games.length; i++) {
	      if (response.games[i].over === true) {
	  ...
	  }).then(ui.getGameSuccess).catch(ui.getGameFailure);
	};

	var onSignUp = function onSignUp(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.signUp(data).then(ui.signUpSuccess).catch(ui.onSignUpFailure);
	};
	...
	var onSignIn = function onSignIn(event) {
	  ...
	  api.signIn(data).then(function (response) {
	    ...
	    return store.user;
	  }).then(ui.signInSuccess).catch(ui.onSignInFailure);
	};

	var onSignOut = function onSignOut(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.signOut(data).done(ui.signOutSuccess).fail(ui.onSignOutFailure);
	};
	...
	var onNewGame = function onNewGame(event) {
	  ...
	  }).done(ui.onNewGameSuccess).fail(ui.fail);
	  getPlayerRecord();
	};
	...
	var onGetGame = function onGetGame() {
	  ...
	  api.getGame().then(function (response) {
	    ...
	    return store.games;
	  }).done(ui.getGameSuccess).catch(ui.getGameFailure);
	};

	var getUpdateIndex = function getUpdateIndex(index) {
	  return index;
	};
	...
	var getUpdateOver = function getUpdateOver(over) {
	  return over;
	};

	var onUpdatingBoard = function onUpdatingBoard(index, value, over) {
	  // let index = getUpdateIndex();
	  // let value = getUpdateValue();
	  // let over = getUpdateOver();
	  api.updatingBoard(index, value, over).then(function (response) {
	    ...
	    console.log('current game array');
	  }).done(ui.updateBoardSuccess).fail(ui.updateBoardFailed);
	};

	var onGameInitiated = function onGameInitiated(event) {
	  ...
	  var divClassNum = parseInt(eventTargetId);
	  var validMove = board.recordMove(divClassNum, gameBoard);
	  ...
	  if (validMove) {
	    ...
	    if (board.gameOver(gameBoard)) {
	      if (board.isWinner(gameBoard)) {
	        ...
	        var index = getUpdateIndex(divClassNum);
	        var value = getUpdateValue(currPlayer);
	        var over = getUpdateOver(true);
	        ...
	        var _index = getUpdateIndex(divClassNum);
	        var _value = getUpdateValue(currPlayer);
	        var _over = getUpdateOver(true);
	        onUpdatingBoard(_index, _value, _over);
	      ...
	      var _index2 = getUpdateIndex(divClassNum);
	      var _value2 = getUpdateValue(currPlayer);
	      var _over2 = getUpdateOver(false);

	      onUpdatingBoard(_index2, _value2, _over2);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updatingBoard = function updatingBoard(index, value, over) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/' + store.curGameId,
	    method: 'PATCH',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var beginPlay = void 0;

	// const = function() {
	//
	// }

	var hideAllWarnings = function hideAllWarnings() {
	...
	var onNewGameSuccess = function onNewGameSuccess() {
	  hideAllWarnings();
	  $('.message-container').show();
	  $('#new-game-b').hide();
	  $(".player-message").text("");
	...
	var onGetStatsSuccess = function onGetStatsSuccess() {
	...
	var onGetStatsFail = function onGetStatsFail() {
	...
	//////////////////////////OTHER
	/////////////////////////
	var getGameSuccess = function getGameSuccess() {
	...
	var updateBoardSuccess = function updateBoardSuccess() {
	  console.log("Board Successfully Changed.");
	};

	var updateBoardFailed = function updateBoardFailed() {
	...
	module.exports = {
	  ...
	  updateBoardFailed: updateBoardFailed,
	  beginPlay: beginPlay,
